...
namespace UnityEssentials.Peek
{
    ...
    public static class PeekSerializeObject
    {
        ...
        {
            if (target is GameObject gameObject && !string.IsNullOrEmpty(gameObject.scene.name))
            {
        ...
        {
            if (index < 0 || index >= PinnedObjectsInScenesList.arraySize)
            {
                return (null);
            }
            UnityEngine.Object obj = PinnedObjectsInScenesList.GetArrayElementAtIndex(index).GetCustomObject();
            return (obj);
        }
